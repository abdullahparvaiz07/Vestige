...
var _getComputedStyle = function _getComputedStyle(element) {
	return _win$2.getComputedStyle(element.nodeType === Node.DOCUMENT_NODE ? element.scrollingElement : element);
};
...
var ScrollTrigger = /*#__PURE__*/ function() {
	...
	_proto.init = function init(vars, animation) {
		...
		self.refresh = function(soft, force, position, pinOffset) {
			...
			if (_isString$1(parsedEnd) && !parsedEnd.indexOf("+=")) if (~parsedEnd.indexOf(" ")) parsedEnd = (_isString$1(parsedStart) ? parsedStart.split(" ")[0] : "") + parsedEnd;
			else {
			...
			if (isFirstRefresh || prevProgress !== self.progress || containerAnimation || invalidateOnRefresh || animation && !animation._initted) {
				animation && !isToggle && (animation._initted || prevProgress || animation.vars.immediateRender !== false) && animation.totalProgress(containerAnimation && start < -.001 && !prevProgress ? gsap$1.utils.normalize(start, end, 0) : prevProgress, true);
	...
	ScrollTrigger.enable = function enable() {
		...
		if (gsap$1) {
			...
			} else if (_doc$2) _doc$2.addEventListener("DOMContentLoaded", function onLoad() {
				ScrollTrigger.enable();
				_doc$2.removeEventListener("DOMContentLoaded", onLoad);
			});
		}
	};
